...
model.depositWealth = Param(model.symbols)
...
def ComputeFirstStageWealth_rule(model):
    return model.FirstStageWealth - summation(model.allocatedWealth) - model.depositWealth == 0.0 

def ComputeSecondStageWealth_rule(model):
    wealth = sum( (1. + riskyRet[m]) * model.riskyWealth[m] 
                 for m in model.symbols)
    wealth += (1.+ model.riskFreeRet) * model.riskFreeWealth
    return model.SecondStageWealth - wealth == 0

model.ComputeFirstStageWealth_ = Constraint()
