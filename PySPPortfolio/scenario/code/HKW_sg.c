...
int HKW_ScenGen(int const FormatOfMoms, TMatrix const * const p_TarMoms,
	...
	/*
	 * FormatOfMoms
	 * p_TarMoms: target moments, size= 4 * n_rvs
	 * p_TgCorrs, corr. mtx, size= n_rvs * n_rvs
	 * p_Probs, prob. of scenarios vector, size= n_scenarios
	 * p_OutMat, output mtx, size= n_rvs * n_scenarios
	 * MaxErrMom, maximal error for Moments
	 * MaxErrCorr, maximal error for Correlations
	 * TestLevel, output Level
	 * MaxTrial, number of Trials
	 * MaxIter, number of Iterations in HKW algorithm
	 * UseStartValues, read Starting distribution from a given file
	 * 以上為user input所給定的參數
	 */
{
	int i,j,s;
	...
	int nVar = p_OutMat->nrow;	//num. of rvs
	int nScen = p_OutMat->ncol;	// num. of scenarios
	...
	//matrix for Cholesky decomposition, size= n_rvs * n_rvs
	...
	double CubParam[4];        // This is what we want to find (a, b, c, d)
	int cub_iter;
	double cub_error;
	InMom[0] = 1;
	...
	Mat_Init(&Chol_TgCorrs, nVar,nVar); //mtx size= n_rvs * n_rvs

	//Corr mtx為被分解的矩陣，分解後的下三角矩陣存在Chol_TgCorrs
	...
	//p_TarMoms為user input matrix, size=4*n_rvs
	//將TgMoms初始化與p_TarMoms相同大小的matrix
	Mat_InitAsBigAs(&TgMoms, p_TarMoms);

	//transform the original target moments (p_TarMoms->val) to new ones (TgMoms.val)
		...
		if (UseStartValues) {
			...
			for (i=0; i<nVar; i++) {
				...
				if (!StartWithCubic) {
					...
					// target moments (算每一個r.v.的4個原始動差)
					...
					do { // trials - different starting distributions
						...
						switch (start_trial) {
							//從normal distribution N(0,1)中抽取1*n_scenarios的向量，共抽取n_rvs次

						case 1:
								// Start with N(0,1) values, discretizing (FIXED values)
								//TmpOutMat size= n_rvs * n_scenarios
