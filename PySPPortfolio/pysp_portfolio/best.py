...
def best_portfolio(symbols, trans_dates, risk_rois,
                   risk_free_rois, allocated_risk_wealth,
                   allocated_risk_free_wealth, buy_trans_fee,
                   sell_trans_fee,
                   predicted_risk_rois, predicted_risk_free_roi,
                   solver=DEFAULT_SOLVER, verbose=False):
    """
    stage-wise portfolio
    ...
    # concrete model
    instance = ConcreteModel(name="best_portfolio")
    ...
    instance.predicted_risk_rois = predicted_risk_rois
    instance.predicted_risk_free_roi = predicted_risk_free_roi

    # Set
    instance.symbols = np.arange(n_stock)

    # decision variables
    # in each period, we buy or sell stock, shape: (n_stock,)
    ...
    def risk_free_wealth_constraint_rule(model):
        ...
        return (model.risk_free_wealth ==
                (1. + model.risk_free_rois) *
                model.allocated_risk_free_wealth + total_sell - total_buy)
    ...
    def wealth_objective_rule(model):
        risk = sum( (1 + model.predicted_risk_rois[mdx]) *
                     model.risk_wealth[mdx]
                    for mdx in model.symbols)
        risk_free = model.risk_free_wealth
        return (risk + risk_free)

    instance.wealth_objective = Objective(rule=wealth_objective_rule,
                                        sense=maximize)
    # solve
    t1 = time()
    param = "best_{}_{}_m{}".format(
        START_DATE.strftime("%Y%m%d"), END_DATE.strftime("%Y%m%d"), n_stock)
    ...
    if verbose:
        display(instance)
        print ("solve best_portfolio {} OK {:.2f} secs".format(
            param, time() - t1))
    ...
    # extract results
    buy_arr = np.zeros(n_stock)
    sell_arr = np.zeros(n_stock)
    risk_arr = np.zeros(n_stock)
    ...
    for mdx in xrange(n_stock):
        # shape: (stock,)
        buy_arr[mdx] = instance.buy_amounts[mdx].value
        sell_arr[mdx] = instance.sell_amounts[mdx].value
        risk_arr[mdx] = instance.risk_wealth[mdx].value

    # shape: (n_stock,)
    buy_amounts = pd.Series(buy_arr, index=symbols)
    sell_amounts = pd.Series(sell_arr, index=symbols)
    risk_wealth = pd.Series(risk_arr, index=symbols)

    current_wealth = risk_arr.sum() + risk_free_wealth
    ...
    return  {
        ...
        # float
        "risk_free_wealth": risk_free_wealth,
        "current_wealth": current_wealth,
        "next_wealth": instance.wealth_objective(),
    }


class BestPortfolio(PortfolioReportMixin, ValidPortfolioParameterMixin):
    ...
    def get_trading_func_name(self, *args, **kwargs):
        return "Best_m{}".format(self.n_stock)
    ...
    def get_current_buy_sell_amounts(self, *args, **kwargs):
        ...
        results = best_portfolio(
            ...
            self.exp_risk_free_rois.iloc[tdx],
            kwargs['allocated_risk_wealth'].as_matrix(),
            kwargs['allocated_risk_free_wealth'],
            self.buy_trans_fee,
            self.sell_trans_fee,
            self.exp_risk_rois.iloc[tdx+1].as_matrix(),
            self.exp_risk_free_rois.iloc[tdx+1],
        )
        return results
    ...
    def run(self, *args, **kwargs):
        ...
        for tdx in xrange(self.n_exp_period -1):
            ...
            # shape: (n_stock,)
            self.buy_amounts_df.iloc[tdx] = results["buy_amounts"]
            self.sell_amounts_df.iloc[tdx] = results["sell_amounts"]
            self.risk_wealth_df.iloc[tdx] = results["risk_wealth"]
            self.risk_free_wealth.iloc[tdx] = results["risk_free_wealth"]

            print ("[{}/{}], current_wealth: {}".format(
                tdx+1, self.n_exp_period - 1, results['current_wealth']))
        ...
        self.risk_wealth_df.iloc[Tdx] = (
            ...
            self.risk_wealth_df.iloc[Tdx-1])

        self.risk_free_wealth.iloc[Tdx] = (
            (1+self.exp_risk_free_rois.iloc[Tdx])*
            self.risk_free_wealth.iloc[Tdx-1])

        final_wealth = (self.risk_free_wealth.iloc[Tdx] +
                        self.risk_wealth_df.iloc[Tdx].sum())

        # compute transaction fee
        trans_fee_loss = (self.buy_amounts_df.sum() * self.buy_trans_fee +
                          self.sell_amounts_df.sum() * self.sell_trans_fee)
        ...
        print ("{} OK [{}-{}], final_wealth:{}, {:.4f}.secs".format(
