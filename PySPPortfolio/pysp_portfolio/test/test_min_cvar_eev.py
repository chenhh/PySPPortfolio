...
def min_cvar_eev_portfolio(symbols, risk_rois, risk_free_roi,
    ...
    ** only one scenario, the mean scenario vector
    ...
    return {
        ...
        "estimated_eev_var": estimated_eev_var,
        "estimated_eev_cvar": estimated_eev_cvar_arr.mean()
    }


def min_cvar_eev2_portfolio(symbols, risk_rois, risk_free_roi,
                           allocated_risk_wealth, allocated_risk_free_wealth,
                           buy_trans_fee, sell_trans_fee, alpha,
                           predict_risk_rois, predict_risk_free_roi,
                           n_scenario, solver="cplex", verbose=False):
    """
    instead of 2nd optimization, using the first optimization result directly
    """
    ...
    for sdx in xrange(n_scenario):
        ...
        portfolio_value = (sum((1+scen_roi[mdx])* instance.risk_wealth[
            mdx].value for mdx in np.arange(n_stock)) +
                          instance.risk_free_wealth.value)
    ...
    if verbose:
        print "min_cvar_eev2_portfolio OK, {:.3f} secs".format(time() - t0)
...
def test_min_cvar_eev2():
    ...
    predict_risk_free_roi = 0
    t = time()
    results =min_cvar_eev_portfolio(symbols, risk_rois, risk_free_roi,
                          allocated_risk_wealth, allocated_risk_free_wealth,
                          buy_trans_fee, sell_trans_fee, alpha,
                          predict_risk_rois, predict_risk_free_roi,
                          n_scenario)
    print "min_eev {:.4f} secs".format(time()-t)
    t2 = time()
    results2 =min_cvar_eev2_portfolio(symbols, risk_rois, risk_free_roi,
                          allocated_risk_wealth, allocated_risk_free_wealth,
                          buy_trans_fee, sell_trans_fee, alpha,
                          predict_risk_rois, predict_risk_free_roi,
                          n_scenario)
    print "min_eev2 {:.4f} secs".format(time()-t2)

    print results['estimated_eev_cvar']
    print '*'*40
    print results2['estimated_eev_cvar']
    print np.testing.assert_almost_equal(results['estimated_eev_cvar'],
                                         results2['estimated_eev_cvar'],)
